...
public class Voto_giornataDAOJdbc implements Voto_giornataDAO {
	...
	public void save(Voto_giornata voto_giornata) {
		...
		try {
			...
			String insert = "insert into voto_giornata( giornata, giocatore, voto, fantavoto, gol_fatto, "
					...
					+ "ammonito, espulso, gol_vittoria, gol_pareggio) "
					+ "values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			...
			statement.setInt(14, voto_giornata.getGoalVittoria());
			statement.setInt(15, voto_giornata.getGoalPareggio());
	...
	public List<Voto_giornata> findAll(int giornata) {
		...
		try {
			...
			String query = "select * from voto_giornata where giornata=?";
